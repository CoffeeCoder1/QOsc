...
QOscBundle::QOscBundle(const QOscBundle &copy) :
		QOscBundle() { *this = copy; }

QOscBundle::QOscBundle(QOscBundle &&move) noexcept { swap(move); }
...
QOscValue QOscBundle::time() const { return d_ptr->time; }
...
void QOscBundle::package(QIODevice *dev) const { d_ptr->write(dev); }

...
bool QOscValuePrivate::compare(const QOscValuePrivate *a, const QOscValuePrivate *b) {
	...
	switch (a->type) {
		...
		case QOsc::MidiType:
			return QOSC_PRIVATE_EQ(a, b, QOscMidiPrivate, port) && QOSC_PRIVATE_EQ(a, b, QOscMidiPrivate, status) && QOSC_PRIVATE_EQ(a, b, QOscMidiPrivate, data1) && QOSC_PRIVATE_EQ(a, b, QOscMidiPrivate, data2);

...
void interface::initTestCase() {
  ...
  test.setRemoteAddress(remoteAddr);
  test.setRemotePort(remotePort);

  QVERIFY(QTest::qWaitFor([=]() { return test.getIsListening(); }));
  ...
  QCOMPARE(test.getLocalAddress(), localAddr);
  QCOMPARE(test.getLocalPort(), localPort);
  QCOMPARE(test.getRemoteAddress(), remoteAddr);
  QCOMPARE(test.getRemotePort(), remotePort);
...
void interface::test_send() {
  ...
  test.send(b);
  verify_send(QByteArray(
      "#bundle\x00"                              // bundle mark
...
void interface::test_connect_dispatch() {
  ...
  test.connect("/a/b/c", [&](const QOscMessage &m) {
    QCOMPARE(m.pattern(), "/a/?/c");
    QCOMPARE(m.toInt(), 10);
    abc = true;
  });

  test.connect("/a/d/c", [&](const QOscMessage &m) {
    QCOMPARE(m.pattern(), "/a/?/c");
    QCOMPARE(m.toInt(), 10);
    adc = true;
  });
...
void interface::test_readme_server() {
  ...
  //    QObject obj;
  //    iface.connect("/my/other/pattern", &obj, SLOT(mySlot(const QOscMessage&
  //    msg)));

  //    // Or get all messages and dispatch them yourself
  //    QObject dispatcher;

  //    QObject::connect(&iface,      SIGNAL(messageReceived(const
  //    QOscMessage&)),
  //                     &dispatcher, SLOT(dispatchMessage(const
  //                     QOscMessage&)));
...
void interface::test_readme_client() {
  ...
  // They can also be timed, You can specify an absolute Date and Time when you
  // want them to be executed

  QOscBundle bundle;
  bundle.setTime(QDateTime::currentDateTime().addSecs(
      15)); // The bundle is for 15sec later
...
QOscMessage interface::magicMessage() { return QOscMessage("/a/b/c", 10); }
